...
class DiscordREST(RESTAware):
    ...
    async def fetch_emoji(self, guild_id: int, emoji_id: int) -> Emoji:
        ...
        url = GET_GUILD_EMOJI.format(guild_id=guild_id, emoji_id=emoji_id)
        response = await self.http.send_request("GET", url=url, headers=headers)
    ...
    async def create_reaction(
        ...
        if isinstance(emoji, Emoji):
            formatted_emoji = str(await self.fetch_emoji(guild_id=guild_id, emoji_id=emoji.emoji_id))
        else:
            emoji_name, emoji_id = emoji.translate(symbols).split(":")

        if reason is not None:
            headers.update({"X-Audit-Log-Reason": reason})

        await self.http.send_request(
            "PUT", url=CREATE_REACTION.format(channel_id=channel_id, message_id=message_id, emoji=formatted_emoji),
            headers=headers
        )
    ...
    async def fetch_guild(self, guild_id: int, with_counts: bool = False) -> Guild:
        headers = {self.http.AUTHORIZATION: self.token, "Content-Type": self.http.APPLICATION_JSON}
        get_guild_url = GET_GUILD.format(guild_id=guild_id) \
            if not with_counts else GET_GUILD.format(guild_id=guild_id) + "?with_counts=true"
        data = await self.http.send_request(
            "GET", get_guild_url,
            headers=headers
        )
    ...
    async def delete_guild(self, guild_id: int) -> None:
        headers = {self.http.AUTHORIZATION: self.token, "Content-Type": self.http.APPLICATION_JSON}
        await self.http.send_request("DELETE", url=DELETE_GUILD.format(guild_id=guild_id), headers=headers)
    ...
    async def create_guild(
        ...
        data = await self.http.send_request("POST", CREATE_GUILD, headers=headers, data=payload)
